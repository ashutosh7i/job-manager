<template>
  <ion-header>
    <ion-toolbar>
      ...
      <ion-title>{{ runNow ? translate('Run now') : translate('Custom Parameters') }}</ion-title>
      <ion-buttons slot="end">
        <ion-button v-if="runNow" @click="confirmRunNow()" color="primary">{{ translate('Save & Run') }}</ion-button>
...
import { generateJobCustomParameters, hasJobDataError, copyToClipboard } from "@/utils";
  ...
  methods: {
    ...
    async confirmRunNow() {
      const jobAlert = await alertController.create({
        header: this.translate("Run now"),
        message: this.translate('Running this job now will not replace this job. A copy of this job will be created and run immediately. You may not be able to reverse this action.', { space: '<br/><br/>' }),
        buttons: [
          {
